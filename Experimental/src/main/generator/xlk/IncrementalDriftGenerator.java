...
public class IncrementalDriftGenerator extends AbstractOptionHandler implements InstanceStream {

	private static final long serialVersionUID = 201512220202L;
	...
	public IntOption driftRate = new IntOption("drftRate", 'd', "Number of instances in the process of the drift", 1000,
			2, 100000);
	...
	double[] pygxbd; // p(y|x) before drift
	double[] pygxad; // p(y|x) after drift
	...
	// p(x) during drift
	double[] intermediatepx;
	// p(y|x) during drift
	double[] intermediatepygx;
	...
	public Example<Instance> nextInstance() {
		...
		if (this.nInstancesGeneratedSoFar <= this.burnInNInstances.getValue()) {
		...
		} else if (this.burnInNInstances.getValue() < this.nInstancesGeneratedSoFar
				&& this.nInstancesGeneratedSoFar <= this.burnInNInstances.getValue() + this.driftRate.getValue()) {
			if (driftPriors.isSet()) {
				sumpx = generateIntermediatepx();
				px = intermediatepx;
			} else {
				px = pxbd;
				sumpx = sumpxbd;
			...
			if (driftConditional.isSet()) {
				generateIntermediatepygx();
				pygx = intermediatepygx;
			} else {
				pygx = pygxbd;
			}
		} else {
		...
		do {
			if (selectedPxIndex == pLength) {
				System.out.println("There is a error!!! sum=" + sum + " rand=" + rand);
				break;
			}
	...
	protected void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		pygxbd = new double[pLength];
		pygxad = new double[pLength];
		...
		if (driftPriors.isSet()) {
			...
			System.out.println("m px:" + this.computeMagnitudePx(pxbd,pxad));
		} else {
			pxad = pxbd;
		}
		...
		if (driftConditional.isSet()) {
			this.generatePygxad();
			System.out.println("m for pygx:" + this.computeMagnitudePygx(pygxbd, pygxad));
		} else {
			pygxad = pygxbd;
		}

		// initialize intermediatepx and intermediatepygx
		intermediatepx = new double[pLength];
		this.intermediatepygx = new double[pygxbd.length];
		System.arraycopy(pygxbd, 0, intermediatepygx, 0, pygxbd.length);
	}
	...
	private void generatePygxad() {
		...
		if (npygxToChange == 0.0) {
			...
			pygxToChange = r.nextPermutation(pLength, npygxToChange);
	...
	private double computeMagnitudePx(double[] pxbd, double[] pxad) {
		double m = 0.0;
		for (int i = 0; i < pLength; i++) {
	...
	private double computeMagnitudePygx(double[] pygxbd, double[] pygxad) {
		double m = 0.0;
		for (int i = 0; i < pLength; i++) {
	...
	protected double generateIntermediatepx() {
		double sum = 0.0;
		double rate = this.driftRate.getValue();
		double process = (int) (this.nInstancesGeneratedSoFar - this.burnInNInstances.getValue());
		
		for(int i=0;i<this.pLength;i++){
			if(pxbd[i]>pxad[i]){
				this.intermediatepx[i]=pxbd[i]-(process/rate)*(pxbd[i]-pxad[i]);
			}else{
				this.intermediatepx[i]=pxbd[i]+(process/rate)*(pxad[i]-pxbd[i]);
			}
			sum+=this.intermediatepx[i];
		}
		
		//System.out.println("process: "+process+"\t sum: "+sum+"\t mpx: "+this.computeMagnitudePx(pxbd, intermediatepx));
		return sum;
	}

	protected void generateIntermediatepygx() {
		int rate = this.driftRate.getValue();
		int process = (int) (this.nInstancesGeneratedSoFar - this.burnInNInstances.getValue());

		int npygxToChange = Math.round(pygxToChange.length * process / rate);
		for (int i = 0; i < npygxToChange; i++) {
			intermediatepygx[pygxToChange[i]] = pygxad[pygxToChange[i]];
		}
		//System.out.println("change: " + npygxToChange + "\t m:" + this.computeMagnitudePygx(pygxbd, intermediatepygx));
	}
